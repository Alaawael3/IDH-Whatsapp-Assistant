...
class WhatsAppClient:
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=1, max=8))
    async def upload_media(self, file_path: str | Path, mime_type: str) -> str:
        """Uploads a local file to WhatsApp's media endpoint and returns the
        resulting media_id, which can then be referenced by an outbound
        send (e.g. send_audio_message). Required before sending any
        locally-generated media -- WhatsApp doesn't accept raw bytes inline
        ...
        if response.status_code >= 400:
            raise WhatsAppError(f"upload_media failed ({response.status_code}): {response.text}")
        media_id = response.json().get("id")
        if not media_id:
            raise WhatsAppError(f"upload_media response had no id: {response.text}")
        log.info("whatsapp_media_uploaded", file_path=str(path), mime_type=mime_type, media_id=media_id)
        return media_id

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=1, max=8))
    async def send_audio_message(self, to: str, media_id: str) -> None:
        """Sends an audio message referencing a previously-uploaded
        media_id (see upload_media). Sent as a generic audio attachment --
        WhatsApp only renders the "voice note" waveform bubble for OGG/Opus
        audio; mp3 (what Fish Audio's TTS returns) still plays fine, it just
        shows as a regular audio player instead of the waveform UI."""
        url = f"{self._base_url}/{self._phone_number_id}/messages"
        payload = {
            "messaging_product": "whatsapp",
            "recipient_type": "individual",
        ...
        if response.status_code >= 400:
            raise WhatsAppError(
                f"send_audio_message failed ({response.status_code}): {response.text}"
            )
        log.info("whatsapp_audio_message_sent", to=to, media_id=media_id)

    async def send_voice_reply(self, to: str, audio_file_path: str | Path, mime_type: str = "audio/mpeg") -> None:
        """Convenience wrapper: upload_media + send_audio_message in one
        call, since every caller needs both steps together."""
        media_id = await self.upload_media(audio_file_path, mime_type)
        await self.send_audio_message(to, media_id)
